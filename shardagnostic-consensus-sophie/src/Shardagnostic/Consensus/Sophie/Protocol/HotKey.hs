...
module Shardagnostic.Consensus.Sophie.Protocol.HotKey (
    -- * KES Info
    KESEvolution
  , KESInfo (..)
  , kesAbsolutePeriod
...
import qualified Bcc.Crypto.KES as Relative (Period)

import           Shardagnostic.Consensus.Block.Forging (UpdateInfo (..))
...
data KESInfo = KESInfo {
   ...
   ,  kesEndPeriod   :: !Absolute.KESPeriod
      -- ^ Currently derived from 'TOptimumParams':
      -- > kesEndPeriod = kesStartPeriod + toptimumMaxKESEvo
