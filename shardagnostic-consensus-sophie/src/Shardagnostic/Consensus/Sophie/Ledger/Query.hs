...
module Shardagnostic.Consensus.Sophie.Ledger.Query (
...
import           Shardagnostic.Network.Block (Serialised (..), decodePoint,
...
import qualified Bcc.Ledger.Core as LC
import qualified Sophie.Spec.Ledger.API as SL
import qualified Sophie.Spec.Ledger.LedgerState as SL (RewardAccounts)
import qualified Sophie.Spec.Ledger.RewardProvenance as SL (RewardProvenance)
...
import           Shardagnostic.Consensus.Sophie.Protocol (TOptimumState (..))
...
instance SL.OptimumCrypto c => Serialise (NonMyopicMemberRewards c) where
  encode = toCBOR . unNonMyopicMemberRewards
  decode = NonMyopicMemberRewards <$> fromCBOR

data instance BlockQuery (SophieBlock era) :: Type -> Type where
  GetLedgerTip :: BlockQuery (SophieBlock era) (Point (SophieBlock era))
  GetEpochNo :: BlockQuery (SophieBlock era) EpochNo
  ...
  GetNonMyopicMemberRewards
    ...
    -> BlockQuery (SophieBlock era) (NonMyopicMemberRewards (EraCrypto era))
  GetCurrentPParams
    :: BlockQuery (SophieBlock era) (LC.PParams era)
  GetProposedPParamsUpdates
    :: BlockQuery (SophieBlock era) (SL.ProposedPPUpdates era)
  ...
  GetStakeDistribution
    :: BlockQuery (SophieBlock era) (SL.PoolDistr (EraCrypto era))
  ...
  GetUTxOByAddress
    ...
    -> BlockQuery (SophieBlock era) (SL.UTxO era)
  ...
  DebugEpochState
    :: BlockQuery (SophieBlock era) (SL.EpochState era)
  ...
  GetFilteredDelegationsAndRewardAccounts
    ...
    -> BlockQuery (SophieBlock era)
             (Delegations (EraCrypto era), SL.RewardAccounts (EraCrypto era))

  GetGenesisConfig
    :: BlockQuery (SophieBlock era) (CompactGenesis era)

  -- | Only for debugging purposes, we make no effort to ensure binary
  -- compatibility (cf the comment on 'GetCBOR'). Moreover, it is huge.
  DebugNewEpochState
    :: BlockQuery (SophieBlock era) (SL.NewEpochState era)

  -- | Only for debugging purposes, we make no effort to ensure binary
  -- compatibility (cf the comment on 'GetCBOR').
  DebugChainDepState
    :: BlockQuery (SophieBlock era) (SL.ChainDepState (EraCrypto era))

  GetRewardProvenance
    :: BlockQuery (SophieBlock era) (SL.RewardProvenance (EraCrypto era))
  ...
  --
  -- WARNING: when adding a new query, a new @SophieNodeToClientVersionX@ must
...
pattern GetUTxO :: BlockQuery (SophieBlock era) (SL.UTxO era)
...
instance Typeable era => ShowProxy (BlockQuery (SophieBlock era)) where

instance SophieBasedEra era => QueryLedger (SophieBlock era) where
  answerBlockQuery cfg query ext =
      case query of
        GetLedgerTip ->
          sophieLedgerTipPoint lst
        GetEpochNo ->
          SL.nesEL st
        ...
        DebugEpochState ->
          getEpochState st
        GetCBOR query' ->
          mkSerialised (encodeSophieResult query') $
            answerBlockQuery cfg query' ext
        GetFilteredDelegationsAndRewardAccounts creds ->
          getFilteredDelegationsAndRewardAccounts st creds
        GetGenesisConfig ->
          sophieLedgerCompactGenesis lcfg
        DebugNewEpochState ->
          st
        DebugChainDepState ->
          toptimumStateChainDepState (headerStateChainDep hst)
        GetRewardProvenance ->
    ...
    where
      ...
      globals = sophieLedgerGlobals lcfg
      ...
      -- data family, and the 'SophieBasedEra' constraint.
      lst = ledgerState ext
      hst = headerState ext
      st  = sophieLedgerState lst

instance SameDepIndex (BlockQuery (SophieBlock era)) where
...
deriving instance Eq   (BlockQuery (SophieBlock era) result)
deriving instance Show (BlockQuery (SophieBlock era) result)

instance SophieBasedEra era => ShowQuery (BlockQuery (SophieBlock era)) where
  showResult = \case
      GetLedgerTip                               -> show
      GetEpochNo                                 -> show
...
-- | Is the given query supported by the given 'SophieNodeToClientVersion'?
querySupportedVersion :: BlockQuery (SophieBlock era) result -> SophieNodeToClientVersion -> Bool
querySupportedVersion = \case
    ...
    -- WARNING: when adding a new query, a new @SophieNodeToClientVersionX@
    -- must be added. See #2830 for a template on how to do this.
  where
    v1 = SophieNodeToClientVersion1
    v2 = SophieNodeToClientVersion2
    v3 = SophieNodeToClientVersion3
    v4 = SophieNodeToClientVersion4
...
encodeSophieQuery ::
     SophieBasedEra era
  => BlockQuery (SophieBlock era) result -> Encoding
encodeSophieQuery query = case query of
    GetLedgerTip ->
    ...
    GetCBOR query' ->
      CBOR.encodeListLen 2 <> CBOR.encodeWord8 9 <> encodeSophieQuery query'
    GetFilteredDelegationsAndRewardAccounts creds ->
...
decodeSophieQuery ::
     SophieBasedEra era
  => Decoder s (SomeSecond BlockQuery (SophieBlock era))
decodeSophieQuery = do
    ...
    case (len, tag) of
      ...
      (2, 9)  -> (\(SomeSecond q) -> SomeSecond (GetCBOR q)) <$> decodeSophieQuery
      ...
      _       -> fail $
        "decodeSophieQuery: invalid (len, tag): (" <>
        show len <> ", " <> show tag <> ")"

encodeSophieResult ::
     SophieBasedEra era
  => BlockQuery (SophieBlock era) result -> result -> Encoding
encodeSophieResult query = case query of
...
decodeSophieResult query = case query of
