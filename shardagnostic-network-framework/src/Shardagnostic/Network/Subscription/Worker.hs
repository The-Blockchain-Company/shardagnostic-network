...
module Shardagnostic.Network.Subscription.Worker
  ( SocketStateChange
  , SocketState (..)
  , CompleteApplication
  , ConnectResult (..)
...
import           Shardagnostic.Network.ErrorPolicy (CompleteApplication, Result (..), CompleteApplicationResult (..), WithAddr, ErrorPolicyTrace)
      ...
      case r of
        ConnectionTableCreate ->
          case wpSelectAddress remoteAddr localAddresses of
            ...
            -- 'Shardagnostic.Network.Server.Socket.spawnOne', it should not
...
-- | Almost the same as 'Shardagnostic.Network.Server.Socket.mainLoop'.
