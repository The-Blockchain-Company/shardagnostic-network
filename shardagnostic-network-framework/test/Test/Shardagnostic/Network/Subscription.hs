...
module Test.Shardagnostic.Network.Subscription (tests) where
...
import           Shardagnostic.Network.Protocol.Handshake.Unversioned
import           Shardagnostic.Network.Protocol.Handshake.Version
...
import           Shardagnostic.Network.Subscription.PeerState
import           Shardagnostic.Network.Subscription.Subscriber
import           Shardagnostic.Network.Subscription.Worker (LocalAddresses(..), WorkerParams(..))

import           Test.Shardagnostic.Network.Orphans ()
...
prop_resolv lr =  do
    ...
    x <- dnsResolve nullTracer (return lr) withMockResolver peerStatesVar (\_ _ s -> pure (AllowConnection s)) $ DnsSubscriptionTarget "sophie-1.tbco.example" 1 2
...
prop_sub_io lr = ioProperty $ withIOManager $ \iocp -> do
    ...
    dnsSubscriptionWorker'
      ...
      SubscriptionParams {
          ...
          spSubscriptionTarget = DnsSubscriptionTarget "sophie-0.tbco.example" 6062 (lrioValency lr)
        }
      (\_ -> do
        ...
        $ \_ _ -> do
          dnsSubscriptionWorker'
            ...
            SubscriptionParams {
                ...
                spErrorPolicies = nullErrorPolicies,
                spSubscriptionTarget = DnsSubscriptionTarget "sophie-0.tbco.example" 6062 1
              }
...
local-data: "sophie-1.tbco.example. IN A 192.168.1.115"
local-data: "sophie-1.tbco.example. IN A 192.168.1.215"
local-data: "sophie-1.tbco.example. IN A 192.168.1.216"
local-data: "sophie-1.tbco.example. IN A 192.168.1.100"
local-data: "sophie-1.tbco.example. IN A 192.168.1.101"
local-data: "sophie-1.tbco.example. IN A 127.0.0.1"
local-data: "sophie-1.tbco.example. IN AAAA ::1"

local-data: "sophie-0.tbco.example. IN AAAA ::1"
-}
_demo :: Property
            ...
            SubscriptionParams {
                spLocalAddresses =
                  LocalAddresses
                    ...
                    Nothing,
                spConnectionAttemptDelay = \_ -> Just minConnectionAttemptDelay,
                spSubscriptionTarget = DnsSubscriptionTarget "sophie-0.tbco.example" 6064 1,
                spErrorPolicies = nullErrorPolicies
