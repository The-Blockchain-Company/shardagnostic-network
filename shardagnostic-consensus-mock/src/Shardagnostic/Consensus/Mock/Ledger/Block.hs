...
module Shardagnostic.Consensus.Mock.Ledger.Block (
    BlockQuery (..)
  , Header (..)
  , SimpleBlock
  , SimpleBlock' (..)
...
import           Bcc.Crypto.Hash (Hash, HashAlgorithm, SHA256, ShortHash)
...
import           Shardagnostic.Consensus.Mock.Ledger.State
import qualified Shardagnostic.Consensus.Mock.Ledger.UTxO as Mock
...
data SimpleLedgerConfig c ext = SimpleLedgerConfig {
      -- | Config required by the various kinds of mock block (PFT, Optimum, ..)
