...
--
-- Normally this is imported from "Shardagnostic.Consensus.Ledger.Abstract". We
...
class ( -- Requirements on the ledger state itself
  ...
  --
  -- * In Cole, scheduled updates are applied, and the update system state is
  --   updated.
  -- * In Sophie, delegation state is updated (on epoch boundaries).
  --
