...
module Shardagnostic.Consensus.Ledger.Dual (
    Bridge (..)
    -- * Pair types
  , DualBlock (..)
  , DualGenTxErr (..)
...
-- example is EBBs: if the main blocks are real Cole blocks, and the auxiliary
-- blocks are Cole spec blocks, then regular Cole blocks correspond to Cole
-- spec blocks, but EBBs don't correspond to a spec block at all and should
-- leave the Cole spec ledger state unchanged.
--
...
instance Bridge m a => IsLedger (LedgerState (DualBlock m a)) where
  ...
  -- and do so only with the Cole and ColeSpecs ledgers, neither of which have
