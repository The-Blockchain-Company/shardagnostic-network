...
-- | Header validation
module Shardagnostic.Consensus.HeaderValidation (
    revalidateHeader
  , validateHeader
    -- * Annotated tips
  , AnnTip (..)
...
import           Shardagnostic.Network.AnchoredSeq (Anchorable (..))
...
import qualified Shardagnostic.Consensus.Util.CBOR as Util.CBOR
...
-- required, but for instance for Cole we need to know if the previous header
-- was an EBB.
...
class ( HasHeader (Header blk)
  ...
  -- "Shardagnostic.Consensus.HardFork.History" for details.
...
--
--   For example, for Optimum this means checking the VRF proofs.
--
