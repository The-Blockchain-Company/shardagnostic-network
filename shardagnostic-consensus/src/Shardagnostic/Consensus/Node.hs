...
--
module Shardagnostic.Consensus.Node (
    run
  , runWith
    -- * Standard arguments
  , StdRunNodeArgs (..)
...
import           Shardagnostic.Network.BlockFetch (BlockFetchConfiguration (..))
import           Shardagnostic.Network.Diffusion
import           Shardagnostic.Network.Magic
import           Shardagnostic.Network.NodeToClient (LocalAddress,
...
import           Shardagnostic.Network.Protocol.Limits (shortWait)

import           Shardagnostic.Consensus.Block
import           Shardagnostic.Consensus.BlockchainTime hiding (getSystemStart)
import           Shardagnostic.Consensus.Config
import           Shardagnostic.Consensus.Config.SupportsNode
import           Shardagnostic.Consensus.Fragment.InFuture (CheckInFuture,
                     ClockSkew)
...
import qualified Shardagnostic.Consensus.Network.NodeToClient as NTC
import qualified Shardagnostic.Consensus.Network.NodeToNode as NTN
import           Shardagnostic.Consensus.Node.DbLock
import           Shardagnostic.Consensus.Node.DbMarker
import           Shardagnostic.Consensus.Node.ErrorPolicy
...
import           Shardagnostic.Consensus.Node.Recovery
import           Shardagnostic.Consensus.Node.Run
import           Shardagnostic.Consensus.Node.Tracers
import           Shardagnostic.Consensus.NodeKernel
import           Shardagnostic.Consensus.Util.Args
...
import           Shardagnostic.Consensus.Util.Time (secondsToNominalDiffTime)

import           Shardagnostic.Consensus.Storage.ChainDB (ChainDB, ChainDbArgs)
import qualified Shardagnostic.Consensus.Storage.ChainDB as ChainDB
import           Shardagnostic.Consensus.Storage.FS.API (SomeHasFS (..))
import           Shardagnostic.Consensus.Storage.FS.API.Types
import           Shardagnostic.Consensus.Storage.FS.IO (ioHasFS)
import           Shardagnostic.Consensus.Storage.ImmutableDB (ChunkInfo,
                     ValidationPolicy (..))
import           Shardagnostic.Consensus.Storage.LedgerDB.DiskPolicy
                     (SnapshotInterval (..), defaultDiskPolicy)
import           Shardagnostic.Consensus.Storage.VolatileDB
...
-- @bcc-node@ will have to be updated, so consider the Node Team's
...
-- 'runWith'. The @bcc-node@, for example, instead calls the 'run'
...
-- some usual assumptions for realistic use cases such as in @bcc-node@.
--
