...
module Shardagnostic.Consensus.Protocol.PBFT.Crypto (
    PBftCrypto (..)
  , PBftMockCrypto
  , PBftMockVerKeyHash (..)
  ) where
...
import           Bcc.Crypto.DSIGN.Mock (MockDSIGN, VerKeyDSIGN (..))

import           Shardagnostic.Consensus.Util.Condense

-- | Crypto primitives required by BFT
--
-- Bcc stores a map of stakeholder IDs rather than the verification key
