...
module Shardagnostic.Consensus.Protocol.Abstract (
    -- * Abstract definition of the Shardagnostic protocol
    ConsensusConfig
  , ConsensusProtocol (..)
  , preferCandidate
...
-- | The (open) universe of Shardagnostic protocols
--
...
class ( Show (ChainDepState   p)
  ...
  -- | Projection of the ledger state the Shardagnostic protocol needs access to
  --
  ...
  -- stake distribution such as Optimum, the 'LedgerView' for a particular slot
  ...
  --
  -- Note that for the specific case of Optimum, whilst the ledger layer provides
...
--
-- If both chains are equally preferable, the Shardagnostic class of consensus
