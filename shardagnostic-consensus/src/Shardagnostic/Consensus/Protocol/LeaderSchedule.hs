...
module Shardagnostic.Consensus.Protocol.LeaderSchedule (
    LeaderSchedule (..)
  , leaderScheduleFor
  ) where
...
import           Shardagnostic.Consensus.NodeId (CoreNodeId (..), fromCoreNodeId)
...
{-------------------------------------------------------------------------------
  ...
  is meant to lead. Unlike in, say, Optimum, where this is determined by a single
