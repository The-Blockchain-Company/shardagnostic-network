...
module Shardagnostic.Consensus.Mempool.TxLimits (
    ByteSize (..)
  , TxLimits (..)
...
import           Shardagnostic.Consensus.Ledger.Abstract (Validated)
import           Shardagnostic.Consensus.Ledger.Basics (LedgerState)
import           Shardagnostic.Consensus.Ledger.SupportsMempool (GenTx)
import           Shardagnostic.Consensus.Ticked (Ticked (..))
...
-- (like Cole and initial generations of Sophie based eras) this
-- measure was simply a ByteSize (block could not be bigger then
-- given size - in bytes - specified by the ledger state). In future
-- eras (starting with Aurum) this measure was a bit more complex
