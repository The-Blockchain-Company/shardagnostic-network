...
module Shardagnostic.Consensus.Util.Condense (
    Condense (..)
  , Condense1 (..)
  , condense1
  ) where
...
import           Bcc.Crypto.DSIGN (Ed25519DSIGN, Ed448DSIGN, MockDSIGN,
...
import           Bcc.Crypto.KES (MockKES, NeverKES, SigKES, SignedKES (..),
...
import           Shardagnostic.Consensus.Util.HList (All, HList (..))
import qualified Shardagnostic.Consensus.Util.HList as HList
...
import           Bcc.Slotting.Slot (EpochNo (..), SlotNo (..),
                     WithOrigin (..))
import           Shardagnostic.Network.Block (ChainHash (..), HeaderHash, Tip (..))
...
{-------------------------------------------------------------------------------
  Orphans for shardagnostic-network
-------------------------------------------------------------------------------}
...
{-------------------------------------------------------------------------------
  Orphans for bcc-crypto-classes
-------------------------------------------------------------------------------}
