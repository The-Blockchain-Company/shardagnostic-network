...
-- | Followers
module Shardagnostic.Consensus.Storage.ChainDB.Impl.Follower (
    closeAllFollowers
  , newFollower
  , switchFork
  ) where
...
import           Shardagnostic.Network.Block (ChainUpdate (..))
...
import           Shardagnostic.Consensus.Util.STM (blockUntilJust)
