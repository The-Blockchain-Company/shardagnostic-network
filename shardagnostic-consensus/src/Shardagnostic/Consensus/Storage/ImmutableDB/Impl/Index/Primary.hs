...
-- > import qualified Shardagnostic.Consensus.Storage.ImmutableDB.Impl.Index.Primary as PrimaryIndex
module Shardagnostic.Consensus.Storage.ImmutableDB.Impl.Index.Primary (
    -- * SecondaryOffset
    SecondaryOffset
    -- * PrimaryIndex
  , PrimaryIndex (..)
...
import           Shardagnostic.Consensus.Block (StandardHash)
...
import           Shardagnostic.Consensus.Storage.FS.API.Types (AbsOffset (..),
...
readFirstFilledSlot pb hasFS@HasFS { hSeek, hGetSome } chunkInfo chunk =
  ...
  where
    ...
    -- Cole-era, the first slot is always filled with an EBB, so we only need
    -- to read one 4-byte offset. In the Sophie era, approximately one in ten
