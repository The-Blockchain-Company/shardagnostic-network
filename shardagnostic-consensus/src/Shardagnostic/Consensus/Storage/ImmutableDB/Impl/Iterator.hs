...
module Shardagnostic.Consensus.Storage.ImmutableDB.Impl.Iterator (
    CurrentChunkInfo (..)
...
import           Shardagnostic.Consensus.Util.ResourceRegistry (ResourceKey,
...
import           Shardagnostic.Consensus.Storage.ImmutableDB.API hiding
                     (getBlockComponent)
...
extractBlockComponent hasFS chunkInfo chunk ccfg checkIntegrity eHnd
  ...
  where
    ...
    -- to read what have written? rmourey_jr says: this is guaranteed at the OS
    ...
    -- file handles ("Shardagnostic.Consensus.Storage.IO") so we're safe (other
