...
module Shardagnostic.Consensus.NodeKernel (
    -- * Node kernel
...
import qualified Shardagnostic.Network.TxSubmission.Inbound as Inbound
import           Shardagnostic.Network.TxSubmission.Mempool.Reader
                     (TxSubmissionMempoolReader)
import qualified Shardagnostic.Network.TxSubmission.Mempool.Reader as MempoolReader

import           Shardagnostic.Consensus.Block hiding (blockMatchesHeader)
import qualified Shardagnostic.Consensus.Block as Block
import           Shardagnostic.Consensus.BlockchainTime
import           Shardagnostic.Consensus.Config
import qualified Shardagnostic.Consensus.Config.SupportsNode as SupportsNode
import           Shardagnostic.Consensus.Forecast
import qualified Shardagnostic.Consensus.HardFork.Abstract as History
...
import           Shardagnostic.Consensus.Util.EarlyExit
...
mkCurrentBlockContext currentSlot c = case c of
    ...
    c' :> hdr -> case blockSlot hdr `compare` currentSlot of
      ...
      --    after @currentTime@).
      --
      -- See <https://github.com/The-Blockchain-Company/shardagnostic-network/issues/1462>
...
-- descending stake.
--
-- For example, for Sophie, this will return the stake pool relays ordered by
-- descending stake.
--
