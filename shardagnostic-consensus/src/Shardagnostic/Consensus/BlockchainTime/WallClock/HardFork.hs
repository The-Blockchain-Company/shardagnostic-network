...
module Shardagnostic.Consensus.BlockchainTime.WallClock.HardFork (
    BackoffDelay (..)
...
import           Shardagnostic.Consensus.BlockchainTime.API
...
import qualified Shardagnostic.Consensus.HardFork.History as HF
...
data HardForkBlockchainTimeArgs m blk = HardForkBlockchainTimeArgs
  ...
  , hfbtMaxClockRewind :: NominalDiffTime
    ...
    --
    -- When the slot length is short, e.g., Optimum' 1s compared to PBFT's 20s,
