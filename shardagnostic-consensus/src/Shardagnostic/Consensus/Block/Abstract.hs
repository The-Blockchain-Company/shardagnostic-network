...
module Shardagnostic.Consensus.Block.Abstract (
    -- * Protocol
    BlockProtocol
    -- * Configuration
  , BlockConfig
  ...
  , succWithOrigin
    -- * Re-export basic definitions from @shardagnostic-network@
  , ChainHash (..)
  , HasHeader (..)
  , HeaderFields (..)
  , HeaderHash
  ...
  , pointHash
  , pointSlot
    -- * Re-export basic definitions from @bcc-base@
  , BlockNo (..)
  , EpochNo (..)
  , EpochSize (..)
  , SlotNo (..)
...
import qualified Bcc.Slotting.Slot as Bcc

import           Shardagnostic.Network.Block (ChainHash (..), HasHeader (..),
...
import           Shardagnostic.Consensus.Block.EBB
...
-- 'Shardagnostic.Consensus.Node.InitStorage.NodeInitStorage' class. Defined here to
