...
module Shardagnostic.Consensus.Block.NestedContent (
    -- * Block contents
    HasNestedContent (..)
  , NestedCtxt_
  , curriedNest
  ...
  , module Shardagnostic.Consensus.Util.DepPair
  , SomeSecond (..)
  ) where
...
import           Shardagnostic.Consensus.Util (SomeSecond (..))
import           Shardagnostic.Consensus.Util.DepPair
...
--
-- Then encoding @Hardfork ColeBlock SophieBlock@ is easy, in the same way
-- that we encode /any/ @Either@. The /header/ of such a block will have type
--
-- > HardFork (Header ColeBlock) (Header SophieBlock)
--
...
-- how do we decode such a header? If it's a Cole block, we should use the
-- decoder for @Header ColeBlock@, and similarly for Sophie, but how should
-- we express this more generally?
--
-- Here is where 'HasNestedContent' comes in. Continuing the example, we can
-- @unnest@ a @Header (HardFork ColeBlock SophieBlock)@ into a pair of values,
...
-- "this is a Cole block", the second value is a @Header ColeBlock@, and vice
