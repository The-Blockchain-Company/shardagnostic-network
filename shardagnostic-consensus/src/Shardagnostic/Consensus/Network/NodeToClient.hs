...
module Shardagnostic.Consensus.Network.NodeToClient (
    -- * Handlers
    Handlers (..)
  , mkHandlers
    -- * Codecs
...
import           Shardagnostic.Network.Block (Serialised, decodePoint, decodeTip,
...
import           Shardagnostic.Network.NodeToClient hiding
                     (NodeToClientVersion (..))
import qualified Shardagnostic.Network.NodeToClient as N (NodeToClientVersion (..))
...
import           Shardagnostic.Network.Protocol.LocalStateQuery.Codec
import           Shardagnostic.Network.Protocol.LocalStateQuery.Server
import           Shardagnostic.Network.Protocol.LocalStateQuery.Type
import           Shardagnostic.Network.Protocol.LocalTxSubmission.Codec
...
import           Shardagnostic.Consensus.MiniProtocol.LocalStateQuery.Server
...
-- point we want to introduce local protocols that for example send Cole blocks
-- or headers across, we will need to have the epoch size, which comes from the
-- Cole config. Unlike the full 'TopLevelConfig', it should not be difficult
-- for a wallet to construct the 'BlockConfig'.
--
-- NOTE: Somewhat confusingly, 'pcChainSyncCodec' currently /does/ send Cole
