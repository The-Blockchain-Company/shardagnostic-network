...
module Shardagnostic.Consensus.MiniProtocol.ChainSync.Client (
    ChainDbView (..)
...
import           Shardagnostic.Network.Block (Tip, getTipBlockNo)
...
import           Shardagnostic.Consensus.HeaderValidation hiding (validateHeader)
...
import           Shardagnostic.Consensus.Util.Assert (assertWithMsg)
import           Shardagnostic.Consensus.Util.IOLike
import           Shardagnostic.Consensus.Util.STM (Fingerprint, Watcher (..),
...
-- can forecast.
--
-- For PBFT (Cole), we can forecast up to @2k@ slots ahead. Assuming a chain
-- density of 100%, this means the look-ahead is @2k@ headers. For mainnet this
-- means @2 * 2160 = 4320@ headers.
--
-- For TOptimum (Sophie), we can forecast up to @3k/f@ slots ahead. Assuming a
...
--
-- For PBFT this is @2160 + 4320 = 6480@ headers, for TOptimum this is @2160 +
...
{-------------------------------------------------------------------------------
  ...
  Scribbled notes during meeting with rmourey_jr:
