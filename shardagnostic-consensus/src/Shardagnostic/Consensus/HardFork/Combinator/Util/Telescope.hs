...
module Shardagnostic.Consensus.HardFork.Combinator.Util.Telescope (
    -- * Telescope
    Telescope (..)
  , sequence
    -- ** Utilities
...
-- state. The "how to extend" argument would take, say, the final Cole
-- state to the initial Sophie state; and "where to extend from" argument
-- would indicate when we want to extend: when the current slot number has
-- gone past the end of the Cole era.
