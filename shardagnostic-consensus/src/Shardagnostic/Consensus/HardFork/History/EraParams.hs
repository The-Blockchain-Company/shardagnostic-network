...
module Shardagnostic.Consensus.HardFork.History.EraParams (
    -- * API
    EraParams (..)
  , SafeZone (..)
    -- * Defaults
...
{-------------------------------------------------------------------------------
  ...
  "certain"; i.e., for Cole, it would mean an update proposal has been voted
  on, confirmed, endorsed, and that endorsement is at least @k@ blocks deep into
  the chain; for Sophie it means an update proposal is voted on and accepted,
  ...
  enough to cover @k@ blocks (and therefore a safe zone of @2k@ slots for Cole,
  and (probably) a safe zone of @3k/f@ slots for Sophie). Effectively, this
