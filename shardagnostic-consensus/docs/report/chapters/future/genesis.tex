...
Recall the Optimum chain selection rule:
...
The Optimum security analysis \cite{cryptoeprint:2017:573} tells us that provided
...
``checkpoint'' in the consensus literature), the Optimum rule from that point
forward would prevent them from (permanently) adopting the wrong chain, but
Optimum cannot be used to help nodes ``bootstrap'' when they are behind.

So far we have just been discussing Optimum as it is described in theory. The
...
involve a rollback of more than $k$ blocks, which the Optimum rule forbids.
...
nodes can (re)join, and the Shardagnostic Genesis paper \cite{cryptoeprint:2018:378}
...
\begin{proof}
The Shardagnostic Genesis analysis \cite{cryptoeprint:2018:378} tells us that
...
\begin{proof}
This is an easy corollary of the Shardagnostic Genesis analysis. If the adversary
...
$f$ & Active slot coefficient (\cref{optimum:f}) \\[1em]
\end{tabular}
...
While the algorithms described in the literature on Shardagnostic compare entire
...
what it would mean to apply, say, the Optimum rule in one era on the chain,
