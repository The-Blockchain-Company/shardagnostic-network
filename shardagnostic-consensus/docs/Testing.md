...
[repository](https://github.com/The-Blockchain-Company/shardagnostic-network/); at the time
of writing, this is

* `shardagnostic-consensus-test/test-infra/Main.hs`
* `shardagnostic-consensus-test/test-storage/Main.hs`
* `shardagnostic-consensus-test/test-consensus/Main.hs`
* `shardagnostic-consensus/shardagnostic-consensus-mock-test/test/Main.hs`
* `shardagnostic-consensus-cole-test/test/Main.hs`
* `shardagnostic-consensus-sophie-test/test/Main.hs`
* `shardagnostic-consensus-bcc/test/Main.hs`
...
The library `shardagnostic-consensus-test` provides a bunch of test utilities that
...
**Stats.** The `shardagnostic-consensus-test` library itself is 8,700 lines of code.
...
### The file system abstraction (`Test.Shardagnostic.Storage.FS.StateMachine`)
...
### The Immutable DB (`Test.Shardagnostic.Storage.ImmutableDB`)
...
#### The primary index (`Test.Shardagnostic.Storage.ImmutableDB.Primary`)
...
#### Model based testing (`Test.Shardagnostic.Storage.ImmutableDB.StateMachine`)
...
The model (defined in `Test.Shardagnostic.Storage.ImmutableDB.Model`) is essentially
...
### The Volatile DB (`Test.Shardagnostic.Storage.VolatileDB.StateMachine`)
...
The model (defined in `Test.Shardagnostic.Storage.VolatileDB.Model`) is a list
...
### The Ledger DB (`Test.Shardagnostic.Storage.LedgerDB`)
...
#### In-memory (`Test.Shardagnostic.Storage.LedgerDB.InMemory`)
...
#### On-disk (`Test.Shardagnostic.Storage.LedgerDB.OnDisk`)
...
### The Chain DB (`Test.Shardagnostic.Storage.ChainDB`)
...
#### The GC schedule (`Test.Shardagnostic.Storage.ChainDB.GcSchedule`)
...
#### Iterators (`Test.Shardagnostic.Storage.ChainDB.Iterator`)
...
#### Properties of the model (`Test.Shardagnostic.Storage.ChainDB.Model.Test`)

The model for the chain DB (`Test.Shardagnostic.Storage.ChainDB.Model`) contains
...
#### Model based testing (`Test.Shardagnostic.Storage.ChainDB.StateMachine`)
...
wallclock separately, because the Shardagnostic protocol says such blocks should
...
* A mock ledger (containing bare bones UTxO style transactions) using a variety
  of consensus protocols: BFT, PBFT, Optimum, and a version of Optimum where we
  override the leader schedule.
* Cole/PBFT
* DualCole: This runs the Cole ledger and the Cole specification in lockstep,
...
* `HardForkBlock '[Cole, Sophie]`: the hard fork combinator instantiated with
  the Cole and Sophie ledgers, running `PBFT` before the transition and
  `TOptimum` after.
...
* Cole integration: 3000 loc.
  Integration of the dual ledger, comparing Cole to the Cole spec: 1200 loc.
  The Cole tests (pure cole, dual cole, but excluding golden tests): 3800 loc.

* Sophie integration: 2750 loc.
...
* Bcc block integration (i.e., HFC applied to Cole-Sophie): 1700 loc.
  Tests: 700 loc.
