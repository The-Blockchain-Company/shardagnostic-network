...
- This change log is primarily intended to benefit downstream Bcc
  ...
  most likely be the `bcc-node` release notes.

- So far, versioning of "internal" Bcc packages is based only on commit
  hashes, except for the `bcc-node` repo.
...
- `git diff revOld revNew -- shardagnostic-consensus/docs/interface-CHANGELOG.md`
...
- `git blame [--first-parent] --
  shardagnostic-consensus/docs/interface-CHANGELOG.md`
...
- `AuxLedgerEvent` type family and interface functions for ticking and applying
  ...
  `bcc-ledger-specs`. These new functions merely extend the Consensus
...
- `shardagnostic-consensus/docs/interface-CHANGELOG.md`. Now that this file exists,
...
- The `*MaxTxCapacityOverrides` fields to the `ProtocolParams*` record types,
  one per ledger era (eg `coleMaxTxCapacityOverrides`,
  `evieMaxTxCapacityOverrides`, etc). In particular, these record types are
  used to organized the lost list of arguments to
  `Shardagnostic.Consensus.Bcc.Node.protocolInfoBcc`. Downstream libraries
  invoke that function.

  Integration advice: These fields have type
  `Shardagnostic.Consensus.Mempool.TxLimits.Overrides` and therefore can be
...
- On-the-wire content of Aurum transactions. Previously they were sent across
  ...
  our nascent Aurum functionalities.

  The new ubiquity of this flag also led to simplifications in the interface to
  `bcc-ledger-specs`. As a result Consensus removed all use of `TxInBlock`,
  ...
  A new feature could later be added eg to the Bcc CLI so that the client
