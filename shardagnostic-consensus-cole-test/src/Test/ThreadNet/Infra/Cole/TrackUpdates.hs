...
module Test.ThreadNet.Infra.Cole.TrackUpdates (
...
import qualified Bcc.Chain.Cole.API as ColeAPI
import qualified Bcc.Chain.Genesis as Genesis
import qualified Bcc.Chain.MempoolPayload as MempoolPayload
import           Bcc.Chain.Slotting (EpochSlots (..), SlotNumber (..))
import qualified Bcc.Chain.Update as Update
import           Bcc.Chain.Update.Proposal (AProposal)
...
import           Bcc.Chain.Update.Vote (AVote)
import qualified Bcc.Chain.Update.Vote as Vote
import qualified Bcc.Crypto as Crypto

import           Shardagnostic.Consensus.Block
import           Shardagnostic.Consensus.Config
import           Shardagnostic.Consensus.Node.ProtocolInfo (NumCoreNodes (..),
                     ProtocolInfo (..))
...
import           Test.ThreadNet.Infra.Cole.ProtocolInfo
...
mkUpdateLabels params numSlots genesisConfig nodeJoinPlan topology result
  ...
  where
    ...
    -- according to the Cole Chain Density invariant
    twoK :: SlotNo
      ...
      o:os -> case o of
          ...
          Ref.Nominal -> case st of
              ...
              Voting proposalSlot votes    ->
                  ...
                  in
                  if  -- TODO bcc-ledger-cole checks for quorum before it checks
    ...
    pvuLabel = ProtocolVersionUpdateLabel
        { pvuObserved =
            ...
            ColeAPI.applyChainTick genesisConfig sentinel $
            Cole.coleLedgerState ldgr
...
--
-- The transactions implement a smoke test for the hard-fork from Cole to
-- Sophie. See PR #1741 for details on how that hard-fork will work. The key
-- fact is that last thing the nodes will ever do while running the Cole
...
-- protocol version.
--
-- Adopting a new protocol version requires four kinds of event in Cole.
...
--    transactions. Instead, every Cole header includes a field that specifies
...
mkProtocolColeAndHardForkTxs
  ...
  -> TestNodeInitialization m ColeBlock
mkProtocolColeAndHardForkTxs
  ...
  where
    ...
    pInfo :: ProtocolInfo m ColeBlock
    opKey :: Crypto.SigningKey
    (pInfo, Crypto.SignKeyColeDSIGN opKey) =
        mkProtocolCole params cid genesisConfig genesisSecrets

    proposals :: [Cole.GenTx ColeBlock]
    proposals =
        if cid /= CoreNodeId 0 then [] else
        (:[]) $
        Cole.fromMempoolPayload $
        MempoolPayload.MempoolUpdateProposal proposal

    votes :: [Cole.GenTx ColeBlock]
    votes =
        (:[]) $
        Cole.fromMempoolPayload $
...
mkHardForkProposal params genesisConfig genesisSecrets propPV =
    ...
    Proposal.signProposal
      ...
      (Crypto.noPassSafeSigner opKey)
  where
    pInfo :: ProtocolInfo Identity ColeBlock
    opKey :: Crypto.SigningKey
    (pInfo, Crypto.SignKeyColeDSIGN opKey) =
        mkProtocolCole params (CoreNodeId 0) genesisConfig genesisSecrets
...
--
loopbackAnnotations
  :: ( Bcc.Binary.FromCBOR (f Bcc.Binary.ByteSpan)
     , Bcc.Binary.ToCBOR (f ())
     , Functor f
     )
  => f ()
