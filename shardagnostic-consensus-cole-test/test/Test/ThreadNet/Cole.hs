...
module Test.ThreadNet.Cole (
    tests
    -- * To support the DualCole tests
  , TestSetup (..)
  , colePBftParams
  , expectedCannotForge
  , genTestSetup
...
import qualified Shardagnostic.Network.MockChain.Chain as Chain
...
import qualified Shardagnostic.Consensus.Protocol.PBFT.Crypto as Crypto
...
import qualified Bcc.Chain.Block as Block
...
import           Bcc.Chain.Slotting (EpochNumber (..), unEpochSlots)
import qualified Bcc.Crypto as Crypto
import qualified Bcc.Crypto.DSIGN as Crypto

import qualified Shardagnostic.Consensus.Cole.Crypto.DSIGN as Crypto
...
import           Test.ThreadNet.TxGen.Cole ()
...
import           Test.ThreadNet.Infra.Cole
...
data TestSetup = TestSetup
  ...
  , setupVersion      :: (NodeToNodeVersion, BlockNodeToNodeVersion ColeBlock)
  }
  deriving (Show)
...
-- | An entrypoint used by "Test.ThreadNet.DualCole"
--
-- See the @'Arbitrary' 'Test.ThreadNet.DualCole.SetupDualCole'@ instance.
...
genTestSetup k numCoreNodes numSlots setupSlotLength = do
    ...
    let params = colePBftParams k numCoreNodes

    nodeJoinPlan <- genColeNodeJoinPlan params numSlots
    nodeRestarts <- genNodeRestarts nodeJoinPlan numSlots >>=
                    genNodeRekeys params nodeJoinPlan nodeTopology numSlots

    setupVersion <- genVersion (Proxy @ColeBlock)
...
tests = testGroup "Cole" $
    [ testProperty "trivial join plan is considered deterministic"
        $ \TestSetup{setupK = k, setupTestConfig = TestConfig{numCoreNodes}} ->
          prop_deterministicPlan $ colePBftParams k numCoreNodes
    , adjustOption (\(QuickCheckTests n) -> QuickCheckTests (1 `max` (div n 10))) $
      ...
      -- https://github.com/The-Blockchain-Company/shardagnostic-network/pull/773#issuecomment-522192097
      testProperty "addressed by InvalidRollForward exception (PR #773)" $
          ...
          prop_simple_real_pbft_convergence TestSetup
            ...
            , setupVersion      = (NodeToNodeV_1, ColeNodeToNodeVersion1)
            }
    ...
    , -- Cole runs are slow, so do 10x less of this narrow test
    ...
    , testProperty "exercise a corner case of mkCurrentBlockContext" $
          ...
          --
          -- This failed with @Exception: the first block on the Cole chain
          -- must be an EBB@.
    ...
    , testProperty "repeatedly add the the dlg cert tx" $
          ...
          -- >            [ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 27}, coleHeaderHash = ColeHash {unColeHash = AbstractHash d50e0d2c}}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 28}, coleHeaderHash = ColeHash {unColeHash = AbstractHash 1523de50}}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 30}, coleHeaderHash = ColeHash {unColeHash = AbstractHash 77cb5dda}}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 31}, coleHeaderHash = ColeHash {unColeHash = AbstractHash 7efd3ec2}}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 33}, coleHeaderHash = ColeHash {unColeHash = AbstractHash 8903fa61}}
          -- > {-an EBB-} ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 40}, coleHeaderHash = ColeHash {unColeHash = AbstractHash 43f8067e}}
          -- >            ]))}
          -- >         , _newChain = AnchoredFragment {anchorPoint = BlockPoint (SlotNo 27) d50e0d2c, unanchorFragment = ChainFragment (SFT (fromList
          -- >            [ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 28}, coleHeaderHash = 1523de50}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 30}, coleHeaderHash = 77cb5dda}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 31}, coleHeaderHash = 7efd3ec2}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 33}, coleHeaderHash = 8903fa61}
          -- >            ,ColeHeader {..., coleHeaderSlotNo = SlotNo {unSlotNo = 34}, coleHeaderHash = afa797b4}
          -- >            ]))}}
          --
    ...
    , testProperty "mkDelegationEnvironment uses currentSlot not latestSlot" $
      ...
      -- cert tx (cf @WrongEpoch@). This happened because the Cole transaction
    ...
    , testProperty "bcc-ledger-cole checks for proposal confirmation before it checks for expiry" $
    ...
    , testProperty "only check updates for mesh topologies" $
          ...
          -- 'Test.ThreadNet.Cole.TrackUpdates.checkTopo'.
          --
          ...
          -- endorsement; see Issue 749 in bcc-ledger-cole).
          --
          -- "Test.ThreadNet.Cole.TrackUpdates" does not otherwise
...
prop_simple_real_pbft_convergence TestSetup
  ...
  , setupVersion      = version
    ...
    prop_general PropGeneralArgs
      ...
      , pgaCountTxs            = Cole.countColeGenTxs
  ...
  where
    ...
    testConfigB = TestConfigB
      { forgeEbbEnv = case produceEBBs of
          NoEBBs      -> Nothing
          ProduceEBBs -> Just coleForgeEbbEnv
    ...
    -- Cole has a hard-coded relation between k and the size of an epoch
    epochSize :: EpochSize
    epochSize = fromColeEpochSlots $ kEpochSlots (toColeBlockCount k)
    ...
    finalChains :: [(NodeId, Chain ColeBlock)]
    finalChains = Map.toList $ nodeOutputFinalChain <$> testOutputNodes testOutput

    finalLedgers :: [(NodeId, Cole.LedgerState ColeBlock)]
    ...
    params :: PBftParams
    params = colePBftParams k numCoreNodes

    genesisConfig  :: Genesis.Config
    genesisSecrets :: Genesis.GeneratedSecrets
    (genesisConfig, genesisSecrets) = generateGenesisConfig slotLength params

coleForgeEbbEnv :: ForgeEbbEnv ColeBlock
coleForgeEbbEnv = ForgeEbbEnv
    { forgeEBB = Cole.forgeEBB . configBlock
    }
...
-- has one tine.
--
genColeNodeJoinPlan :: PBftParams -> NumSlots -> Gen NodeJoinPlan
genColeNodeJoinPlan params numSlots@(NumSlots t)
  | n < 0 || t < 1 = error $ "Cannot generate Cole NodeJoinPlan: "
  ...
  | otherwise      =
    go (NodeJoinPlan Map.empty) Ref.emptyState
      `suchThat` (\njp -> Ref.definitelyEnoughBlocks params $
        ...
        -- satisfies it.
        --
        -- In a run of 7000 successful Cole tests, this 'suchThat' retried:
        --
        -- 486 retried once
        -- 100 retried twice
        -- 10 retried 3 times
    ...
    go nodeJoinPlan@(NodeJoinPlan m) st
      ...
      | otherwise = do
        ...
        s' <- case out <$> searchFromTo (check . out) (inn lastSlot) (inn lo) of
            ...
            Nothing -> error $
                "Cannot find viable Cole NodeJoinPlan: " ++
...
-- (Shardagnostic allows at most one delegation per epoch, while each rekey and
...
mkRekeyUpd
  ...
  -> EpochNo
  -> Crypto.SignKeyDSIGN Crypto.ColeDSIGN
  -> m (Maybe (TestNodeInitialization m ColeBlock))
...
genesisSecretFor genesisConfig genesisSecrets cid =
    case hits of
        [sec] -> Crypto.SignKeyColeDSIGN sec
  ...
  where
    ...
    gkToIdx =
      ...
      . Crypto.VerKeyColeDSIGN . Crypto.toVerification

-- | Create new 'ColeLeaderCredentials' by generating a new delegation
...
updSignKey genSK extCfg cid eno newSK =
    ColeLeaderCredentials {
      ...
      , blcDlgCert    = newCert
      , blcCoreNodeId = cid
      , blcLabel      = "Updated Cole credentials"
      }
  where
    newCert =
        Delegation.signCertificate
            (Cole.coleProtocolMagicId extCfg)
            (Crypto.toVerification sk')
            eno
            (Crypto.noPassSafeSigner gsk')

    Crypto.SignKeyColeDSIGN gsk' = genSK
    Crypto.SignKeyColeDSIGN sk'  = newSK

-- | Map a delegation certificate to a delegation transaction
--
dlgTx :: Delegation.Certificate -> Cole.GenTx ColeBlock
dlgTx cert =
    let ann = Bcc.Binary.serialize' (cert :: Delegation.Certificate)
        cert' = cert
          { Delegation.aEpoch     =
              Bcc.Binary.reAnnotate (Delegation.aEpoch cert)
          , Delegation.annotation = ann
          }
    in Cole.ColeDlg (Delegation.recoverCertificateId cert') cert'
