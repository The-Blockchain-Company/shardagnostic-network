...
module Test.ThreadNet.DualCole (tests) where
...
import qualified Bcc.Chain.ProtocolConstants as Impl
...
import qualified Test.Bcc.Chain.Elaboration.UTxO as Spec.Test
...
import           Shardagnostic.Consensus.ColeDual.Node

import qualified Test.ThreadNet.Cole as Cole
...
tests :: TestTree
tests = testGroup "DualCole" [
      testProperty "convergence" $ prop_convergence
    ]

-- These tests are very expensive, due to the Cole generators
-- (100 tests take about 20 minutes)
-- We limit it to 10 tests for now.
prop_convergence :: SetupDualCole -> Property
prop_convergence setup = withMaxSuccess 10 $
    ...
    prop_general PropGeneralArgs
      ...
      , pgaCountTxs            = countColeGenTxs . dualBlockMain
  ...
  where
    ...
    -- bcc-ledger-specs code causes @PBFTFailure (SlotNotAfterLastBlock
    -- (Slot 0) (Slot 0))@ in that case. So we discard such tests.
    --
    -- This is ultimately due to the spec not modeling EBBs, while Cole
...
data SetupDualCole = SetupDualCole {
      setupGenesis :: ColeSpecGenesis
    , setupCole   :: Cole.TestSetup
    }
  deriving (Show)

setupParams :: SetupDualCole -> PBftParams
setupParams = colePBftParams . setupGenesis

setupTestConfigB :: SetupDualCole -> TestConfigB DualColeBlock
setupTestConfigB SetupDualCole{..} = TestConfigB
  ...
  , txGenExtra   = ()
  , version      = newestVersion (Proxy @DualColeBlock)
  }
  where
    Cole.TestSetup{..} = setupCole
    ...
    epochSize :: EpochSize
    epochSize =
        fromColeEpochSlots $ Impl.kEpochSlots (toColeBlockCount setupK)

setupTestOutput :: SetupDualCole -> TestOutput DualColeBlock
setupTestOutput setup@SetupDualCole{..} =
    runTestNetwork testConfig testConfigB TestConfigMB {
        nodeInfo = \coreNodeId ->
          plainTestNodeInitialization $
          protocolInfoDualCole
            setupGenesis
            (setupParams setup)
            [coreNodeId]
  ...
  where
    testConfig  = Cole.setupTestConfig setupCole
...
setupExpectedCannotForge ::
  ...
  -> SlotNo
  -> NodeId
  -> WrapCannotForge DualColeBlock
  -> Bool
setupExpectedCannotForge SetupDualCole{..} s nid (WrapCannotForge cl) =
    Cole.expectedCannotForge
      setupK
      numCoreNodes
      setupNodeRestarts
...
{-------------------------------------------------------------------------------
  Generator for 'SetupDualCole'
-------------------------------------------------------------------------------}

-- | We do an awkward dance in this generator. We want to reuse
-- 'Cole.TestSetup' as much as possible. However, 'genSpecGenesis' needs values
-- provided by 'Cole.TestSetup' (ie @numSlots@ and @slotLen@) but also sets a
-- value provided by 'Cole.TestSetup' (eg @k@).
instance Arbitrary SetupDualCole where
  arbitrary = do
      numSlots <- arbitrary
      slotLen  <- arbitrary

      genesis0                 <- genSpecGenesis slotLen numSlots
      let params@PBftParams{..} = colePBftParams genesis0
      ...
      setupCole <-
        (\x -> x{Cole.setupNodeRestarts = noRestarts})
        <$> Cole.genTestSetup
              pbftSecurityParam
              pbftNumNodes
              numSlots
      ...
      return SetupDualCole{..}
    where
      -- The spec tests and the Cole tests compute a different test value for
      -- the PBFT threshold. For now we ignore the value computed by the spec
      -- and override it with the value computed in the Cole tests.
      --
...
genSpecGenesis :: SlotLength -> NumSlots -> Gen ColeSpecGenesis
genSpecGenesis slotLen (NumSlots numSlots) = fmap fromEnv . hedgehog $
  ...
  where
    -- Start with a larger initial UTxO. This is important, because the Cole
    ...
    fromEnv :: Spec.Environment Spec.CHAIN -> ColeSpecGenesis
    fromEnv = Genesis.modUtxoValues (* 10000)
            . Genesis.fromChainEnv (toColeSlotLength slotLen)

colePBftParams :: ColeSpecGenesis -> PBftParams
colePBftParams ColeSpecGenesis{..} =
    Cole.colePBftParams (SecurityParam k) numCoreNodes
  where
    Spec.BlockCount k = coleSpecGenesisSecurityParam
...
instance TxGen DualColeBlock where
  testGenTxs _coreNodeId _numCoreNodes curSlotNo cfg () = \st -> do
    ...
    where
      ...
      go :: [GenTx DualColeBlock]     -- Accumulator
         -> Integer                    -- Number of txs to still produce
         -> TickedLedgerState DualColeBlock
         -> Gen [GenTx DualColeBlock]
...
-- infrastructure of the Cole tests.
genTx :: TopLevelConfig DualColeBlock
      -> Ticked (LedgerState DualColeBlock)
      -> Gen (GenTx DualColeBlock)
genTx cfg st = do
    ...
    return $ DualGenTx {
        dualGenTxMain   = ColeTx (coleIdTx main) main
      , dualGenTxAux    = ColeSpecGenTx $ ColeSpecGenTxTx aux
  ...
  where
    ...
    st'  = tickedColeSpecLedgerState $ tickedDualLedgerStateAux st

    bridge :: ColeSpecBridge
