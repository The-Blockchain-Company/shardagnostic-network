...
module Shardagnostic.Consensus.Cole.Ledger.Inspect (
    ColeLedgerUpdate (..)
    -- * Layer around the Cole protocol update inteface
  , ProtocolUpdate (..)
  , UpdateState (..)
  , protocolUpdates
  ) where
...
import qualified Bcc.Chain.ProtocolConstants as CC
import qualified Bcc.Chain.Slotting as CC

import qualified Bcc.Chain.Update as U
import qualified Bcc.Chain.Update.Validation.Endorsement as U.E
import qualified Bcc.Chain.Update.Validation.Interface as U.I
import qualified Bcc.Chain.Update.Validation.Registration as U.R

import           Shardagnostic.Consensus.Block
import           Shardagnostic.Consensus.Config
import qualified Shardagnostic.Consensus.HardFork.History.Util as History
...
-- | Wrapper around a Cole protocol update with information about its state
--
...
protocolUpdates genesis st = concat [
  ...
  where
    ...
    updState     = CC.cvsUpdateState $ coleLedgerState st
    ...
    fromRegistered (upId, proposal) = ProtocolUpdate {
      ...
      where
        ...
        updConfirmed = fromColeSlotNo <$> Map.lookup upId confirmed
        updEndorsed  = Map.findWithDefault Set.empty version endorsements
        updSlot      = case Map.lookup upId registeredAt of
                         Nothing   -> error "updSlot: invalid Cole state"
                         Just slot -> fromColeSlotNo slot
    ...
    fromCandidate candidate = ProtocolUpdate {
      ...
      where
        ...
        slot    = fromColeSlotNo $ U.E.cpuSlot candidate
    ...
    -- Slot conversion
    --
    -- This is valid for slots in the Cole era only; just like the Cole
    -- ledger itself, it assumes the Cole era is the /first/ era.
...
data ColeLedgerUpdate =
    ColeUpdatedProtocolUpdates [ProtocolUpdate]
  deriving (Show, Eq)

instance Condense ColeLedgerUpdate where
  condense = show

instance InspectLedger ColeBlock where
  type LedgerWarning ColeBlock = Void
  type LedgerUpdate  ColeBlock = ColeLedgerUpdate

  inspectLedger tlc before after = do
      guard $ updatesBefore /= updatesAfter
      return $ LedgerUpdate $ ColeUpdatedProtocolUpdates updatesAfter
    where
