...
-- | Missing instances for standard type classes in the Cole spec
module Shardagnostic.Consensus.ColeSpec.Ledger.Orphans () where
...
import qualified Cole.Spec.Chain.STS.Rule.Pbft as Spec
import qualified Cole.Spec.Chain.STS.Rule.SigCnt as Spec
...
import           Test.Bcc.Chain.Elaboration.Block as Spec.Test
...
instance Serialise Spec.Entropic
...
{-------------------------------------------------------------------------------
  Not all types in bcc-ledger-specs have generic instances
-------------------------------------------------------------------------------}
...
{-------------------------------------------------------------------------------
  Auxiliary: Bcc.Binary.ToCBOR/FromCBOR to Serialise bridge
-------------------------------------------------------------------------------}

newtype ToFromCBOR a = ToFromCBOR { unToFromCBOR :: a }

instance ( Bcc.Binary.ToCBOR   a
         , Bcc.Binary.FromCBOR a
         ) => Serialise (ToFromCBOR a) where
  encode = Bcc.Binary.toCBOR . unToFromCBOR
  decode = ToFromCBOR <$> Bcc.Binary.fromCBOR
