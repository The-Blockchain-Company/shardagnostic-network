...
module Test.Shardagnostic.Storage.TestBlock (
    -- * Test block
    BlockConfig (..)
  , ChainLength (..)
  , CodecConfig (..)
...
import           Shardagnostic.Consensus.Protocol.ModChainSel
...
import           Shardagnostic.Consensus.Storage.FS.API (HasFS (..), hGetExactly,
...
-- shorter chain.
--
-- Note that this is not a problem for Cole, because we don't look at the
-- hashes or anything else in case of a tie (we just prefer a chain ending
-- with an EBB, which /must/ be longer).
--
-- Note that is not a problem for Sophie either, where we do look at the
