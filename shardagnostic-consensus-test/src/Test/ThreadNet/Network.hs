...
import           Shardagnostic.Network.Codec (AnyMessage (..), CodecFailure,
                     mapFailureCodec)
import qualified Shardagnostic.Network.Codec as Codec
import           Shardagnostic.Network.MockChain.Chain (Chain (Genesis))
import           Shardagnostic.Network.Point (WithOrigin (..))
import qualified Shardagnostic.Network.Protocol.ChainSync.Type as CS

import           Shardagnostic.Network.Mux (ControlMessage (..), ControlMessageSTM)
import           Shardagnostic.Network.NodeToNode (MiniProtocolParameters (..))
import           Shardagnostic.Network.Protocol.KeepAlive.Type
import           Shardagnostic.Network.Protocol.Limits (waitForever)
...
import           Shardagnostic.Consensus.Mempool
import qualified Shardagnostic.Consensus.MiniProtocol.ChainSync.Client as CSClient
...
import           Shardagnostic.Consensus.NodeKernel as NodeKernel
...
import qualified Shardagnostic.Consensus.Storage.ImmutableDB.Impl.Index as Index
import qualified Shardagnostic.Consensus.Storage.LedgerDB.DiskPolicy as LgrDB
...
runThreadNetwork systemTime ThreadNetworkArgs
  ...
  , tnaBlockVersion   = blockVersion
    ...
    -- @nodeTopology@.
    --
    -- Each graph vertex is a node operator, and maintains its own Shardagnostic
