...
import qualified Shardagnostic.Network.MockChain.Chain as MockChain

import           Shardagnostic.Consensus.Block
import qualified Shardagnostic.Consensus.Block.Abstract as BA
import qualified Shardagnostic.Consensus.BlockchainTime as BTime
import           Shardagnostic.Consensus.Config.SecurityParam
import           Shardagnostic.Consensus.Ledger.Extended (ExtValidationError)
...
import           Shardagnostic.Consensus.Protocol.Abstract (LedgerView)
import           Shardagnostic.Consensus.Protocol.LeaderSchedule
...
-- absence/lateness. And 'epochSize' is here because eg the Cole ledger
...
data PropGeneralArgs blk = PropGeneralArgs
  ...
  , pgaFirstBlockNo        :: BlockNo
    ...
    --
    -- At time of writing this comment... For example, this is 1 for Cole
  ...
  , pgaFixedMaxForkLength  :: Maybe NumBlocks
    ...
    -- 'Shardagnostic.Consensus.Protocol.PBFT.PBftExceededSignThreshold'.
    --
...
--
-- Late Join Assumption: TODO! Only Shardagnostic Genesis is designed to have nodes
...
--     effectively includes the epoch 0 EBB for Cole).
--
...
--
-- Specific tests make additional assumptions, eg the @Cole@ tests make
...
-- same slot in which it was sent. The Shardagnostic research papers instead
