...
import           Shardagnostic.Consensus.Block (BlockProtocol, CodecConfig, Header,
...
import           Shardagnostic.Consensus.Ledger.Query (BlockQuery, QueryVersion,
...
import           Shardagnostic.Consensus.Node.Run (SerialiseDiskConstraints,
...
import           Shardagnostic.Consensus.Storage.Serialisation (EncodeDisk (..),
                     SerialisedHeader)
import           Shardagnostic.Consensus.Util.CBOR (decodeAsFlatTerm)
...
-- serialising a Sophie header in combination with
-- 'BccNodeToNodeVersion1', we 'show' the exception and use that as the
-- output.
goldenTest_all ::
