...
Sophie is a first fully decentralised release of Bcc system, implementing the Shardagnostic protocol.
...
This document discusses the high level functional and performance requirements for Shardagnostic and the
...
How this work relates (in general terms) to the rest of the Sophie
development and the Shardagnostic papers.

\subsection{Data Diffusion assumptions in Shardagnostic}
\begin{quote}
...
\item Communication between Sophie Nodes and existing core nodes.
\item Cole proxy
\end{itemize}
...
\begin{itemize}
\item IPC between Bcc nodes that are engaged in the high level Shardagnostic
      blockchain consensus protocol.
\item IPC between a Bcc node and a `chain consumer' component such as a
...
,while the first version of Bcc used a single protocol, the new version will
...
\wip{
  Here only an outline. Full discussion in the Section~\ref{shardagnostic}.
\begin{description}
