...
\hypersetup{
  pdftitle={Introduction to the design of the Data Diffusion and Networking for Bcc/Sophie},
  pdfborder={0 0 0},
  breaklinks=true
}
...
\title{Introduction to the design of the
       ...
       Bcc Sophie\footnote{Supplemented by a Part 2 which is a
       Reference Document aimed at implementers of Shelly.
  \href{https://hydra.tbco.io/build/1011577/download/1/network.pdf}{{https://hydra.tbco.io/build/1011577/download/1/network.pdf}}} \\
       {\large \sc An TBCO technical report}
  }
\date{Version 1.9, August 2020}
\author{rmourey_jr Coutts      \\ {\small \texttt{duncan@well-typed.com}} \\
   ...
   \and Marcin Szamotulski \\ {\small \texttt{marcin.szamotulski@blockchain-company.io}}
   \and Peter Thompson     \\ {\small \texttt{peter.thompson@pnsol.com}} \\
                              {\small \texttt{peter.thompson@blockchain-company.io}}
   }
...
networking layer of the Bcc Sophie implementation of Shardagnostic
Optimum. The key functional role of this layer is the data diffusion of
blocks, transactions and(and other information to be shared) in Bcc
...
requirements provided at the project's outset then refined with the TBCO
business team.

We give design requirements, compare with existing protocols, and
describe the rationale for the solution adopted by the TBCO Networking
Team.

\paragraph{Who the original was for.}
It was intended to provide a semi-technical, high-level overview of the Bcc
Sophie networking layer to inform strategic decision making and to
...
We build and expand on the design requirements from the Shardagnostic Optimum
paper. It links with the Sophie Ledger specification.

The networking solution is being deployed as part of the Sophie
...
Shardagnostic, differ from those of a Proof-of-Work (PoW) system, such as
Bitcoin or Ethereum.
...
Shardagnostic distributed consensus algorithm: chain transmission,
...
evolution of Shardagnostic. It includes a network protocol framework that
...
Bcc is a third-generation blockchain\footnote{Bitcoin was the first
  generation. Ethereum is an example of a second generation blockchain.}
based on a new proof-of-stake consensus algorithm called Shardagnostic
\cite{BGKR17, BGKRZ19}. Bcc's first implementation,
\href{https://bccroadmap.com/en/cole/}{{Cole}}, was introduced in
September 2017.

Sophie is a full, modular, reimplementation of Bcc, organised into
two main functional components:

\begin{enumerate}
\item
  the \emph{data diffusion} layer, which governs how data propagates
  between Bcc nodes, and
\item
  the \emph{consensus} layer, which implements the Shardagnostic algorithm
  and applies the ledger rules.
\end{enumerate}

Bcc is a distributed algorithm. Consequently in the Sophie
implementation each network node runs a Sophie \emph{instance}, whose
functions interact via the data diffusion component.

This document describes, for the Sophie implementation of Bcc:
...
\item
  ...
  implementation of the Shardagnostic algorithm, and summarise the key
...
\item
  ...
  Shardagnostic Optimum (\cref{performance-model-of-shardagnostic-optimum}); and
  give a detailed description of the software components that are
  required by our solution (\cref{shardagnostic-network-components}).
\end{enumerate}
...
decisions of the network layer of Bcc. However, the network layer
design is fundamentally shaped by

\begin{itemize}
\item
  the constraints and design of the Bcc consensus layer, and
\item
...
Bcc as a cryptocurrency system fundamentally relies on an
implementation of Shardagnostic \cite{BGKR17, BGKRZ19}. The underlying
properties of Shardagnostic\footnote{Specifically, liveness and persistence
  -- see below.} ensure, \emph{by design,} that Bcc users can submit
...
It follows that a key part of the commercial offering is that Bcc,
being based on an implementation of Shardagnostic, can actually achieve
...
The key requirements for the Shardagnostic implementation come from the
Shardagnostic specification as embodied in the published and peer-reviewed
\href{https://tbco.io/research/papers/}{{papers}}, and from business
...
The mathematical description of the Shardagnostic algorithm is not intended
...
Shardagnostic establishes strong properties of \emph{progress} (liveness)
and \emph{persistence}\footnote{These two properties are the
  underpinning of the Bcc cryptocurrency ledger.} , based on
...
\paragraph{The Shardagnostic specification}
\label{the-shardagnostic-specification}

Shardagnostic \emph{as an algorithm specification} is elegantly modular,
comprising:
...
All current members of the Shardagnostic family have this same structure,
which suggests using a parameterized consensus design that can be
instantiated for BFT, Optimum and the other variants\footnote{The new
  Cole-compatible consensus algorithm is Shardagnostic ``Permissive'' BFT,
  a custom variation designed to handle the transition from Shardagnostic
  Classic to Shardagnostic BFT. The Sophie consensus algorithm is the
  sequential composition of Shardagnostic Permissive BFT followed by the
  combination of Shardagnostic Optimum with a transitional BFT overlay
  schedule.} that are used in Bcc Cole and Bcc Sophie.
...
simply eats into the ``adversarial stake budget''. TBCO's Shardagnostic
...
invalid chains for Shardagnostic. The validation check for Shardagnostic relies
on having a full and (nearly) up-to-date ledger state. Having a full
ledger state depends on the other two pieces of Shardagnostic functionality,
...
requires the Shardagnostic chain selection and chain validation features,
...
PoS-based systems such as Shardagnostic.
...
By contrast, with PoS in Shardagnostic:
...
\item
  ...
  Shardagnostic functionality: chain validation and chain selection.
\end{itemize}

Thus we conclude that the Shardagnostic broadcast functionality cannot be
implemented in isolation, and that an integrated design is required that
combines all three areas of Shardagnostic functionality: chain broadcast,
validation and selection.

\paragraph{Bounding the resource use of honest nodes}

An important consideration in making a more detailed design for
Shardagnostic stems from the nature of the proof of the Shardagnostic
...
\cref{the-shardagnostic-specification} the
...
the three parts of the Shardagnostic algorithm functionality:
...
Our design interleaves Shardagnostic functionality in the following sense:
...
in the path. After the first design refinement\footnote{This design is
  an intermediate design refinement. Like the high level Shardagnostic
...
\cref{the-shardagnostic-specification},
...
In the case of Shardagnostic, we can pack all the cryptographic consensus
...
blockchain algorithms such as Shardagnostic have \emph{eventual} finality,
...
The validation and chain selection functionalities of Shardagnostic are in
four separate places:

\begin{enumerate}
\item
  Peer chain header validation is performed in the chain sync client.
  This covers all the Shardagnostic chain validation protocol checks, but
...
requirements of the consensus design\footnote{Though the implementation
  ...
  checks needed to implement many instances of the Shardagnostic family.}, a
...
Shardagnostic design. After all, Shardagnostic itself is a high-level
...
\begin{itemize}
\item
  The consensus layer handles issues deriving from the Shardagnostic
  algorithm, whereas
\item
...
\item
  ...
  disadvantage in meeting the Shardagnostic timing constraints.
\end{itemize}
...
\begin{enumerate}
\item
  the Shardagnostic timing constraints;
\item
...
itself that scores well on all the criteria\footnote{Thrift and
  ...
  the new Bcc implementation.}. eusing the same choice leads to
significant savings:
...
This is true for the present Bcc implementation and any other
...
unusual: the old bcc-sl network implementation also relied on a
...
be used in any Shardagnostic implementation that respects the Shardagnostic
...
not useful for an implementation of Shardagnostic data diffusion as
discussed in
...
fully distributed Bcc implementation. This is discussed in summary
...
also typically asynchronous, unlike Shardagnostic which must ensure block
delivery within deadlines.

Bitcoin, Ethereum and the previous Bcc implementation are considered
in more detail in
...
architecture for the Bcc node. This architectural style is designed,
...
Shardagnostic. The business requirements are set out in
...
Shardagnostic is a system with hard real-time deadlines. They are deadlines
...
deadlines destroys the properties of the system\footnote{Optimum does make
...
deadlines in the vast majority of cases. TBCO's Chief Scientist Aggelos
...
details). This is not an option for Shardagnostic as it exists now: we must
...
\begin{itemize}
\item
  the network layer should be decentralised in the sense that "TBCO
...
(megabyte) blocks. In the existing Cole system the smallest practical
...
Shardagnostic does not share these difficulties. While it remains the case
...
\cref{performance-model-of-shardagnostic-optimum}
...
resistance to eclipse attacks. As described above, however, Shardagnostic
...
initially from nodes contributed by TBCO and later by state pool
...
process. See the
\href{https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/delegationDesignSpec/latest/download-by-type/doc-pdf/delegation_design_spec}{{Sophie
...
\subsection{Characteristics of Bcc}
\label{characteristics-of-bcc}

We start with a PoS algorithm called \emph{Shardagnostic}, and a
...
Bcc -- and its Sophie implementation in particular -- refines
Shardagnostic\footnote{Sophie includes an array of proofs and tests to
  ensure that desirable properties of the Shardagnostic algorithm are
...
globally distributed network of nodes implementing Shardagnostic in the real
...
Bcc is ultimately a cooperating system of autonomous nodes. It is
...
However, its original implementation codenamed `Cole' has seven
federated nodes that produce blocks.
\cref{decentralisation-design} is
the goal of the second phase of Sophie.

\subsection{Fundamental requirements of Bcc data diffusion}
\label{fundamental-requirements-of-bcc-data-diffusion}

The Sophie network component has a specific problem to solve: it must
...
sub-systems outside Bcc's control\footnote{Any blockchain protocol
...
The time-slotted nature of the Shardagnostic algorithm introduces a strong
...
of delivery of every individual block, effectively making Bcc a
...
potential to arm an adversary to disrupt Bcc.
...
interval\footnote{This is the exact interval for Shardagnostic Classic, and the
targeted average interval for Optimum.}. Note that each `hop' includes both
...
or not? We consider some evidence from Bitcoin, Ethereum, and Bcc.
...
Bcc, however, is consistently delivering blocks to (at least) the
...
possible to run Bcc for a couple of months before forks would appear
...
\item
  The appearance of relay networks such as FIBRE\footnote{Note that this
    is essentially the approach that we are taking with Bcc for the
...
block before passing it on to peers, but for Bcc, more expensive
...
Ethereum}}. The graph suggests that Bcc's requirement of
...
and the slot length in Bcc is not straightforward. In Ethereum (or
...
one of \emph{those} nodes will win the race. But in Bcc, the node
...
this specific node\footnote{In Shardagnostic Optimum, the node due to be
...
mainly in the ballistic throughput range of TCP\footnote{The ballistic
  ...
  the block size used.}, so a hop in Ethereum is fast. For Bcc, we
...
The third data point is the first implementation of Bcc itself.
...
Kademlia-based network implementation\footnote{Note that there was a
  Kademlia implementation in the original Bcc-SL code, and it was
...
used in Cole -- 7 tightly connected core nodes that produce blocks,
...
handle the unique requirements of Bcc.
...
Fundamentally, the threat model is that of Shardagnostic, with some
...
The Shardagnostic assumption is quite weak: simply that at least 50\% of the
stake is held by agents that will follow the Shardagnostic algorithm
faithfully\footnote{Technically, in Shardagnostic Optimum, this assertion is
...
We also have the requirement that anyone can participate in the Bcc
...
The principal threats considered in the Sophie network design are:
...
The response to these in the Sophie network design is summarised in
\cref{summary-response-to-threats}

\subsubsection{Adversarial peers}
\label{adversarial-peers}

The Bcc system consists of independently operating nodes that
...
peers are adversarial)\footnote{This is consistent with the use of the
  term in the Shardagnostic papers, but applied to the network protocols
  between peers.}.

The Shardagnostic algorithm works to deliver reliable consensus even when
...
In PoS systems such as Shardagnostic, however, blocks are generated
...
blocks\footnote{Complications in eclipse detection arising from the
  random generation of blocks in Shardagnostic Optimum are discussed in
...
the security guarantee of Shardagnostic.
...
By contrast, in a PoS blockchain such as Bcc it is cheap for
...
However, for Bcc, provided the whole system of nodes is sufficiently
...
that Bcc as a whole cannot be easily compromised is one of the
...
\item
  ...
  \begin{itemize}
  \item
    And the risk of failing to do so in the Sophie delivery timescale?
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    ...
    implement Shardagnostic but did not produce anything safe, whereas using
...
\item
  ...
  in Bcc is essentially free, and our network will not be very
  large, this is not a strong defence and certainly would invalidate our
  Shardagnostic claims, based only on the Shardagnostic assumptions\footnote{Our
...
Such anonymity enhancement is not a requirement for Sophie, but could
...
Bcc does not need this because the stake-based leader selection
...
above the use of gossiping) are not required for Bcc.
...
performance and security of Bcc. This is evidenced by the block
...
\item
  ...
  \begin{itemize}
  \item
    This was included in the bcc-sl codebase but proved unsuitable
...
\item
  ...
  \end{itemize}
\item
  Delivers no specific benefits to Bcc
...
small fraction of nodes in the system\footnote{In the Facebook and
  ...
  10,000 nodes.}. This is not a good fit for Shardagnostic which requires
...
As discussed in \cref{the-shardagnostic-specification}, pub/sub systems
...
Shardagnostic represents a pathological case from its design perspective. It
...
absent in the Shardagnostic use case. It should be noted \cite[Section 5.4]{SSVV12}
...
the messages from another peer, which is essential for Bcc in order
...
thousand) compared to the target population size for Bcc --
...
\end{minipage} & \begin{minipage}[t]{0.22\columnwidth}\raggedright
Resistance to eclipse (irrelevant in Bcc)
...
Bcc Sophie should be realisable on various platforms and scales of
...
Bcc Sophie should also be usable by enterprises, so it must meet
...
Shardagnostic Optimum will aim for the same rate of block production as
Shardagnostic Classic\footnote{We assume that relaxing these constraints
substantially -- which would lead to Sophie having much lower
...
slot time of 1s and a slot occupancy of 5\%. Allowing the Optimum $\Delta$
...
transactions sizes in the 8k+ range. Envisaged future uses (Zerepoch /
...
In common with the design of Shardagnostic, we assume that any Bcc peer
...
delegation design\footnote{https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/delegationDesignSpec/latest/download-by-type/doc-pdf/delegation\_design\_spec}
that roughly {100 - 1000 peers} will hold at least 80\% of the total stake.

\subsection{Graceful degradation}
\label{graceful-degradation}

In order to maximise the chances that Bcc/Sophie will survive a
...
While Sophie aims to be more efficient than Cole, it must still be
possible to communicate between the two different versions of Bcc in
...
More generally, in the expectation that Bcc will continue to evolve,
...
Creating an efficient implementation of an algorithm\footnote{For
  Shardagnostic, an example would be broadcasting blocks rather than
...
In the Sophie iteration we will not aim to formally prove this. It is,
...
would be catastrophic for Bcc, however, as it would enable an
...
block-producing nodes operated by TBCO, by surrounding them with
sacrificial relays.
...
something to the other at the same time. The solution in Sophie is
...
which allows it to be used for different shardagnostic protocols like
Shardagnostic-BFT or Shardagnostic-Optimum. Applying types to protocols rather
...
\cref{decentralisation-constraints}, Bcc nodes
...
requirement for Bcc is to limit the number of hops blocks must
...
Sophie fundamentally improves on Cole by having a \emph{compositional}
...
overhead, both in bytes over the network and in CPU cycles\footnote{Note
  ...
  effectively run a Bcc node, as discussed in
  \cref{fundamental-tradeoffs}}.

\subsubsection{Structured information exchanges}
\label{structured-information-exchanges}

A naive implementation of Shardagnostic would transmit far more data than
...
\item
  ...
  \item
    ...
    mathematical convenience to formulate the Shardagnostic algorithm in
...
\href{https://hydra.tbco.io/build/1011577/download/1/network.pdf}{{second
...
and is required for the fully decentralised version of Sophie. The
...
network layer alone. These scenarios break the assumptions of Shardagnostic
and handling them may require Shardagnostic protocol modifications, and may
...
integrity of Bcc might be at risk.
...
behaviour in Shardagnostic/Bcc related to the operation of stakepools.
...
Bcc Foundation could help manage through a suitable conformance /
...
As a Klarity home user with my stake delegated to other users I would
like to join the Bcc network so I can participate in the network.
...
As a Klarity home user operating a node with a small stake, I would
like to join the Bcc network so I can participate in the network as
...
network, I would like to join the Bcc network so I can participate
...
As a home user, I want to see a network connection status on Klarity so
that I know the state of my network connection.

\begin{itemize}
\item
  If the user receives a notification that they are in red or amber
  mode, Klarity will give the user some helpful information on how to
...
This assumes that the slot time remains 20 seconds\footnote{Under Optimum
...
\item
  ...
  Shardagnostic which states that the users that control 50\% of the stake
  are non-adversarial.
\end{enumerate}
...
As a user I want to be assured that TBCO and its business partners are
...
\begin{itemize}
\item
  TBCO should be in the same position on the network as any other
...
In Sophie we are using (pre-established) TCP/IP bearers, over which
...
\item
  ...
  window)\footnote{See Section 4.1 in
    ...
    Section 2). For Bcc deployments within AWS this has a credible upper
...
\subsection{Performance model of Shardagnostic Optimum}
\label{performance-model-of-shardagnostic-optimum}
...
Shardagnostic Optimum (in common with the original `classic' version) assumes
...
\cite{BGKR17}, since Bcc is otherwise potentially vulnerable to
...
Leap seconds do not pose a problem to Optimum by themselves; however, the
...
level of `adversarial' behavior that Optimum can easily withstand.
...
Thus, even with all Bcc nodes using NTP, local variations in the
...
assumption in the Optimum paper that differences between local clocks are
...
this offset changes whenever a leap-second is introduced. If a Bcc
...
\subsection{Shardagnostic Network Components}
\label{shardagnostic-network-components}

The \emph{Shardagnostic Network} implementation consists of the following
Haskell packages:
...
\item
  \texttt{shardagnostic-network-framework} -- a library which makes low
  level choice for \emph{shardagnostic-network} and implements low level
  network components / primitives, e.g. connection manager, server.
\item
  \texttt{shardagnostic-network} -- a library which implements the consensus
...
presented\footnote{The framework was presented by rmourey_jr Coutts at
...
\subsubsection{Shardagnostic-Network}
\label{shardagnostic-network}

The \emph{shardagnostic-network} library implements \emph{mini-protocols}
...
design flaw in the Cole era, which is using tcp-sockets and thus needs
...
Shardagnostic protocol is being used, and all the protocols reflect that
...
\paragraph{Other components of Shardagnostic-Network package}
...
This is a small part of the Shardagnostic-Network package which implements
an efficient chain data structure: `AnchoredFragment`.
It is hidden at the bottom of the stack since it is useful in the
`BlockFetch` component and in various \emph{Shardagnostic-Network} tests.
...
very useful testing the \emph{shardagnostic-consensus} package maintained by
the consensus team.
