...
-- protocol using the 'Shardagnostic.Network.MockChain.Chain' module.  The module
-- is used to build tests on randomly generated graphs of nodes, see
-- 'Test.Shardagnostic.Network.MockNoder'.
--
-- A historical note: this was the very first node implemntation that pre-dates
-- 'typed-protocols'.
--
module Shardagnostic.Network.MockNode where
...
import           Shardagnostic.Network.MockChain.ProducerState
                     (ChainProducerState (..), initChainProducerState,
                     producerChain, switchFork)
import           Shardagnostic.Network.Point (WithOrigin (At))
...
-- legit useful.
import           Shardagnostic.Network.Testing.ConcreteBlock hiding (fixupBlock)
import qualified Shardagnostic.Network.Testing.ConcreteBlock as Concrete
