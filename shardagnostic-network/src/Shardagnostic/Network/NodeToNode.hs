...
--
module Shardagnostic.Network.NodeToNode (
    nodeToNodeProtocols
  , NodeToNodeProtocols (..)
...
import           Shardagnostic.Network.Driver.Simple (DecoderFailure)
import           Shardagnostic.Network.Driver.Limits (ProtocolLimitFailure (..))
import           Shardagnostic.Network.IOManager
import           Shardagnostic.Network.Mux
import           Shardagnostic.Network.NodeToNode.Version
import           Shardagnostic.Network.ErrorPolicy
import           Shardagnostic.Network.BlockFetch.Client (BlockFetchProtocolFailure)
import qualified Shardagnostic.Network.TxSubmission.Inbound as TxInbound
import qualified Shardagnostic.Network.TxSubmission.Outbound as TxOutbound
import           Shardagnostic.Network.Socket
import           Shardagnostic.Network.Tracers
import           Shardagnostic.Network.Protocol.Handshake.Type
import           Shardagnostic.Network.Protocol.Handshake.Codec
import           Shardagnostic.Network.Protocol.Handshake.Version hiding (Accept)
import           Shardagnostic.Network.Subscription.Ip (IPSubscriptionParams, SubscriptionParams (..))
import qualified Shardagnostic.Network.Subscription.Ip as Subscription
import           Shardagnostic.Network.Subscription.Ip ( IPSubscriptionTarget (..)
                                                   , WithIPList (..)
                                                   , SubscriptionTrace (..)
                                                   )
import           Shardagnostic.Network.Subscription.Dns (DnsSubscriptionParams)
import qualified Shardagnostic.Network.Subscription.Dns as Subscription
import           Shardagnostic.Network.Subscription.Dns ( DnsSubscriptionTarget (..)
                                                    , DnsTrace (..)
                                                    , WithDomainName (..)
                                                    )
import           Shardagnostic.Network.Subscription.Worker (LocalAddresses (..), SubscriberError)
import           Shardagnostic.Network.Snocket
  ...
  ArkApplication $ \connectionId controlMessageSTM ->
   ...
   where
    ...
    blockFetchProtocolLimits = MiniProtocolLimits {
        ...
        -- 'Shardagnostic.Network.BlockFetch.blockFetchLogic' (where
        ...
        -- ```
        --
        -- In the cole era this limit was set to `10 * 2MB`, we keep the more
        -- relaxed limit here.
        --
    ...
    txSubmissionProtocolLimits = MiniProtocolLimits {
          ...
          -- (defined in -- 'Shardagnostic.Network.TxSubmission.Inbound.txSubmissionInbound')
          --
...
-- | A specialised version of @'Shardagnostic.Network.Socket.connectToNode'@.
--
connectTo
...
-- | A specialised version of @'Shardagnostic.Network.Socket.withServerNode'@.
...
-- raised by `shardagnostic-network`.
--
