...
-- look like.
module Shardagnostic.Network.Block (
    SlotNo(..)
  , BlockNo(..)
  , HeaderHash
  , HeaderFields(..)
...
import           Bcc.Binary (Case (..), Size, szCases, szGreedy)

import           Bcc.Slotting.Block
import           Bcc.Slotting.Slot (SlotNo (..))

import           Shardagnostic.Network.Point (WithOrigin (..), block,
                     fromWithOrigin, origin, withOriginToMaybe)
import qualified Shardagnostic.Network.Point as Point (Block (..))
...
-- instantiations of 'blk', e.g. 'ColeBlock, etc.  Thus this test should live
-- in 'shardagnostic-consensus'.
...
encodedTipSize encodedHeaderHashSize tipProxy =
  ...
  -- 'encodedSizeExpr', also include a test in `bcc-ledger-cole`.
...
-- 'bcc-consensus' where similar tests exists (and all the infrastructure
