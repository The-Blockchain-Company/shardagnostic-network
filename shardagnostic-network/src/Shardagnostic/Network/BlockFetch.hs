...
-}
module Shardagnostic.Network.BlockFetch (
    blockFetchLogic,
...
import           Shardagnostic.Network.BlockFetch.State
import           Shardagnostic.Network.BlockFetch.ClientState (FromConsensus(..))
...
data BlockFetchConsensusInterface peer header block m =
     BlockFetchConsensusInterface {
       ...
       -- candidate chain. Classically for Shardagnostic this would simply
       -- check if the candidate is strictly longer, but for Shardagnostic
