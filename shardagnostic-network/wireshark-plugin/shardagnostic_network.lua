...
local shardagnostic = Proto("Shardagnostic", "Shardagnostic")

local on_transmission_time = ProtoField.new ("Transmission Time", "shardagnostic.ts", ftypes.UINT32)
local on_length = ProtoField.new ("Length", "shardagnostic.length", ftypes.UINT16)
...
local on_conversation = ProtoField.uint16("shardagnostic.conv", "Conversation", base.HEX, conv_ids, nil, "Conversation ids")
...
local on_handshake_msg = ProtoField.uint8("shardagnostic.handshakemsg", "Handshake Message", base.DEC, handshake_msg_codes, nil, "Handshake Message Types")
...
local on_chainsync_msg = ProtoField.uint8("shardagnostic.chainmsg", "ChainSync Message", base.DEC, chainsync_msg_codes, nil, "ChainSync Message Types")
...
local on_blockfetch_msg = ProtoField.uint8("shardagnostic.blockmsg", "BlockFetch Message", base.DEC, blockfetch_msg_codes, nil, "BlockFetch Message Types")
...
local on_txsubmission_msg = ProtoField.uint8("shardagnostic.txsubmsg", "TxSubmission Message", base.DEC, txsubmission_msg_codes, nil, "TxSubmission Message Types")
...
local on_keepalive_msg = ProtoField.uint8("shardagnostic.keepalivemsg", "KeepAlive Message", base.DEC, keepalive_msg_codes, nil, "KeepAlive Message Types")


shardagnostic.fields = {
...
function shardagnostic.dissector(tvbuf, pktinfo, root)

	local pktlen = tvbuf:len()
	local offset = 0

	while offset < pktlen do
		result = dissectArk(tvbuf, pktinfo, root, offset)
		if result > 0 then
			-- We parsed an Shardagnostic message
...
dissectArk = function (tvbuf, pktinfo, root, offset)
	...
	pktinfo.cols.protocol = shardagnostic.name

	local subtree = root:add(shardagnostic, tvbuf(), "Shardagnostic")
...
tcp_port:add(6061, shardagnostic)
