...
import qualified Shardagnostic.Consensus.HardFork.History.Util as Util
...
import qualified Bcc.Protocol.TOptimum.Rules.Overlay as SL
...
-- Cole) lasts for one epoch and the second era (eg Sophie) lasts for an
...
genPartition (NumCoreNodes n) (NumSlots t) (SecurityParam k) = do
    ...
    --   /if/ /given/ /enough/ /time/ /alone/. But for Cole at least, the
    ...
    --   speed until the partition starts). (TODO does that cover Sophie too?)
    --
      ...
      frequency $
        ...
        filter (maybe True (< t) . snd)
          ...
          , (20, assert (numFirstEraEpochs == (1 :: Int)) $
                 Just $ coleEpochSize (SecurityParam k))
          ]
...
sophieEpochSize :: SecurityParam -> Word64
sophieEpochSize k = unEpochSize $ Sophie.mkEpochSize k activeSlotCoeff
