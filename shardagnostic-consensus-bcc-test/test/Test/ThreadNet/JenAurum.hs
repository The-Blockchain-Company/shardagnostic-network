...
module Test.ThreadNet.JenAurum (tests) where
...
import           Test.ThreadNet.TxGen.Aurum ()
import           Test.ThreadNet.TxGen.Jen ()
...
type JenAurumBlock =
  SophieBasedHardForkBlock (JenEra Crypto) (AurumEra Crypto)
...
data TestSetup = TestSetup
  ...
  , setupVersion      :: (NodeToNodeVersion, BlockNodeToNodeVersion JenAurumBlock)
  }
  deriving (Show)
...
tests :: TestTree
tests = testGroup "JenAurum ThreadNet" $
    [ let name = "simple convergence" in
      ...
      testProperty name $ \setup ->
        prop_simple_evieAurum_convergence setup
    ]

prop_simple_evieAurum_convergence :: TestSetup -> Property
prop_simple_evieAurum_convergence TestSetup
  { setupD
  , setupHardFork
  , setupInitialNonce
  , setupK
  ...
  where
    ...
    testOutput :: TestOutput JenAurumBlock
          ...
          nodeInfo = \(CoreNodeId nid) ->
            TestNodeInitialization {
                tniCrucialTxs   =
                  ...
                  fmap GenTxSophie1 $
                  Sophie.mkMASetDecentralizationParamTxs
                    coreNodes
    ...
    genesisJen :: SophieGenesis (JenEra Crypto)
    genesisJen =
    ...
    overlaySlots =
        secondEraOverlaySlots
          numSlots
          (NumSlots numFirstEraSlots)
          (SL._d (sgProtocolParams genesisJen))
          epochSize
