...
module Test.ThreadNet.EvieJen (tests) where
...
type EvieJenBlock =
  SophieBasedHardForkBlock (EvieEra Crypto) (JenEra Crypto)
...
data TestSetup = TestSetup
  ...
  , setupVersion      :: (NodeToNodeVersion, BlockNodeToNodeVersion EvieJenBlock)
  }
  deriving (Show)
...
tests :: TestTree
tests = testGroup "EvieJen ThreadNet" $
    [ let name = "simple convergence" in
      ...
      testProperty name $ \setup ->
        prop_simple_evieJen_convergence setup
    ]

prop_simple_evieJen_convergence :: TestSetup -> Property
prop_simple_evieJen_convergence TestSetup
  { setupD
  , setupHardFork
  , setupInitialNonce
  , setupK
  ...
  where
    ...
    testOutput :: TestOutput EvieJenBlock
    ...
    genesisSophie :: SophieGenesis (EvieEra Crypto)
    genesisSophie =
